...
def FW_nuclear_attack(model, images, labels, radius=1, eps=1e-10, step_size=0.1, iters=2, device='cpu'):
    criterion = torch.nn.CrossEntropyLoss()
    ...
    for iteration in range(iters):
        ...
        v_FW = nuclear_norm(-grad, radius, device=device)
        ...
        if torch.norm(grad) < eps:
            break
    adv_images = torch.clamp(images.detach() + perturbation.detach(), 0, 1)
    return adv_images

# ---------------------------------spectral_norm--------------------------------------------
def spectral_norm(D: torch.Tensor, radius: float, device='cpu') -> torch.Tensor:
    assert isinstance(D, torch.Tensor), "Input must be a PyTorch tensor."
    # 假设 'D' 已经是一个至少三维的张量，形状可能是 (C, H, W) 或 (B, C, H, W)
    if D.ndim == 3:
...
def FW_spectral_attack(model, images, labels, radius=1, eps=1e-10, step_size=1.0, iters=40, device='cpu'):
    ...
    for iteration in range(iters):
        ...
        # Compute optimal perturbation
        v_FW = spectral_norm(-grad, radius, device=device)
        step_size = step_size * 1.0 / (iteration + 1)
        perturbation.data += step_size * (v_FW - perturbation)
